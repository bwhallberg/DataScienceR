...
# Go to the directory with all the data
setwd("~/Documents/Rstudio/DataScienceR/DataAnalysis-Exercise-2")

# Read in the data
adult <- read.sas7bdat("adult.sas7bdat")
...
ggplot(adult, aes(x = SRAGE_P1, fill=factor(RBMI),col=factor(RBMI))) + 
  geom_histogram(binwidth=1)

#
#  Cleanup data that seems to be outliers
#
...
#  For now, we'll look deeper at frequency histograms and begin developing our mosaic plots.
...
#  Since ggplot does not do the frequecy histogram correctly with facets, build what we need and
#  try again.
...
# 
# Now working on the Mosaic Plot Code
#
...
#
#  OK, now lets add some text to the plots.
#
...
#
# Everything before this was exercises to get to this function and then these sample plots of data
#
# Load all packages
library(ggplot2)
library(reshape2)
